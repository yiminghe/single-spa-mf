...
const App1Home = ({ publicPath, singleSpa }) => {
  ...
  return (<div>
    <BrowserRouter><div>
      <Routes>
        <Route path={publicPath + 'app1'} element={
          <>
            <div>app1 home</div>
        ...
        <Route path={publicPath + "app1/intro"} element={
          <>
            <div>app1 intro</div>
            <Button onClick={gotoHome}>goto app1 home</Button>
          </>
        } />
        <Route path="*" element={<Navigate to={publicPath + 'app1'} replace />} />
      </Routes>
    </div></BrowserRouter>
  </div>);
};
